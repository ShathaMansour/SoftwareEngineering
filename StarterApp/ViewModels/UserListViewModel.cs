...
public partial class UserListViewModel : INotifyPropertyChanged
{
    ...
    public UserListViewModel(AppDbContext context, INavigationService navigationService, IAuthenticationService authenticationService)
    {
        ...
        RefreshCommand = new AsyncRelayCommand(async () => await RefreshUsersAsync());
        UserSelectedCommand = new AsyncRelayCommand<UserListItem>(async (user) => await NavigateToUserDetailAsync(user));
        CreateUserCommand = new AsyncRelayCommand(async () => await NavigateToCreateUserAsync());
